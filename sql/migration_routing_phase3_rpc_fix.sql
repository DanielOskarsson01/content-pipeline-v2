-- ============================================================
-- Phase 3 RPC Fix: apply_entity_routing accepts p_routing_step
-- ============================================================
-- routingHandler.js calls with 3 params:
--   { p_run_id, p_routing_decisions, p_routing_step }
-- but the Phase 2 RPC only accepts 2. This causes every
-- routing call to fail with a Supabase parameter error.
--
-- Fix: add p_routing_step with DEFAULT 10 (matches the legacy hardcoded
--      value and the JS-side default of routingStep = 10 in routingHandler.js).
--      All callers supply the value explicitly; DEFAULT is a safety net only.
-- Also: use p_routing_step in entity_routing_log instead of
--       the hardcoded 10.
-- Note: two other hardcoded 10s remain in the body (lines ~87, ~206) —
--       those are intentional pipeline-ceiling constants (step_index <= 10),
--       not the routing step, and are unchanged from Phase 2.
-- ============================================================
...
BEGIN
    ...
    CASE v_decision_type
      WHEN 'approve' THEN
        ...
        UPDATE entity_stage_pool SET
          status = 'approved'
        WHERE run_id = p_run_id AND step_index = 10 AND entity_name = v_entity_name; -- 10 = pipeline ceiling (Step 10 pool), not p_routing_step
      ...
      ELSE
        ...
        -- Log the routing decision (use actual routing step, not hardcoded 10)
  ...
  IF v_routed_count > 0 THEN
    ...
    WHERE run_id = p_run_id
      ...
      AND step_index <= 10; -- 10 = pipeline ceiling constant, not p_routing_step
...
-- FROM pg_proc
-- WHERE proname = 'apply_entity_routing';
-- Expected: apply_entity_routing | p_run_id uuid, p_routing_decisions jsonb, p_routing_step integer DEFAULT 7
-- ============================================================
