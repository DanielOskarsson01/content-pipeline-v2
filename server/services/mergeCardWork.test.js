...
const W = 'c9d0e1f2-a3b4-5678-cdef-901234567890'; // content-writer, Round-1, placed @5
const W2 = 'd0e1f2a3-b4c5-6789-def0-123456789abc'; // content-writer-v2, Round-2, routing-only escalation
...
function sourcePlan() {
  return {
    ...
    card_definitions: {
      [W]: { card_name: 'content-writer', submodule_id: 'content-writer', step: 5, round: 1, overrides: {} },
      [W2]: { card_name: 'content-writer-v2', submodule_id: 'content-writer', step: 5, round: 2, overrides: { temperature: 0.3 } },
      [P]: { card_name: 'pse-directories', submodule_id: 'google-pse-curated-search', step: 1, round: 1, overrides: { curated_list: 'directories' } },
    },
    routing_rules: { 'citation:fail': [{ step: 5, card_id: W2 }] },
...
test('carries card work + the also-dropped structural fields from source', () => {
  ...
  assert.deepEqual(executionPlan.routing_rules, { 'citation:fail': [{ step: 5, card_id: W2 }] });
...
test('carried fields are deep-copied — mutating the output cannot corrupt the source plan', () => {
  ...
  executionPlan.card_definitions[W2].overrides.temperature = 999;
  assert.deepEqual(s.skip_steps, [3]);                               // source unchanged
  assert.equal(s.card_definitions[W2].overrides.temperature, 0.3);  // source unchanged
});
