...
// A cached call: tokens_in is only the UNCACHED remainder; the bulk of the
// input arrived as cache_read (prompt-cache hit). content-analyzer does this today.
const cachedCall    = { provider: 'anthropic', model: 'sonnet', tokens_in: 4885, tokens_out: 874, cache_write_tokens: 0, cache_read_tokens: 17483, stop_reason: 'end_turn' };
...
test('cache tokens land per-call and in totals (true input undercount guard)', () => {
  const result = { items: [], meta: {} };
  applyAiCallMeta(result, [cachedCall, completeCall]);
  const u = result.meta.ai_usage;
  // per-call cache fields preserved
  assert.equal(u.calls[0].cache_read_tokens, 17483);
  assert.equal(u.calls[0].cache_write_tokens, 0);
  // totals
  assert.equal(u.cache_read_tokens_total, 17483);
  assert.equal(u.cache_write_tokens_total, 0);
  // tokens_in stays the UNCACHED remainder; cache is tracked separately so
  // true input = tokens_in_total + cache_write_tokens_total + cache_read_tokens_total
  assert.equal(u.tokens_in_total, 4885 + 21834);
  assert.equal(u.tokens_in_total + u.cache_write_tokens_total + u.cache_read_tokens_total, 4885 + 21834 + 17483);
  // a call with no cache fields defaults to 0, never NaN/undefined
  assert.equal(u.calls[1].cache_read_tokens, 0);
});
